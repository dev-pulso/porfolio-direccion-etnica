...
export function Loading() {
  ...
  const loadingSteps = [
    { icon: Landmark, text: "Conectando con el servidor..." },
    { icon: Users, text: "Verificando credenciales..." },
    { icon: FileText, text: "Cargando módulos..." },
    { icon: Scale, text: "Preparando interfaz..." },
  ];
    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 w-full">
      ...
      <div className="flex gap-2 mt-8">
        {loadingSteps.map((step, index) => {
          ...
          return (
            <div
              key={index}
              className={`w-10 h-10 rounded-lg flex items-center justify-center transition-all duration-300 ${index === currentStep
      ...
      <div className="absolute bottom-8 text-center">
        <p className="text-xs text-muted-foreground">
          © {new Date().getFullYear()} Dirección Étnica Municipal
        </p>
      </div>
    </div>
  );
